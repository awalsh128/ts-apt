...
PR-gated CI is split into setup, CodeQL, linting, unit testing, and integration testing jobs.

Trigger policy:

- [.github/workflows/ci.yml](.github/workflows/ci.yml) is the required pull request gate for protected branches such as `main` and `staging`.
- Keep branch protection required checks tied to the PR-gated workflow.
